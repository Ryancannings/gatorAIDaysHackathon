...
class EncoderCNN(nn.Module):
    ...
    def fine_tune(self, fine_tune=False):
        ...
        if fine_tune:
            # Explicitly unfreeze layer4 (the top block)
            layer4 = list(self.resnet.children())[-1]
            for p in layer4.parameters():
                p.requires_grad = True
            print("✅ Fine-tuning enabled: layer4 is trainable")
        else:
            print("🧊 Encoder frozen (no fine-tuning)")
...
def main():
    ...
    encoder = EncoderCNN(encoded_image_size=14, train_cnn=True).to(device)
    decoder = DecoderWithAttention(512, 512, 512, len(vocab.itos), 2048).to(device)

    criterion = nn.CrossEntropyLoss(ignore_index=vocab.stoi["<pad>"])

    # Create optimizers safely
    enc_params = list(filter(lambda p: p.requires_grad, encoder.parameters()))
    if len(enc_params) > 0:
        enc_optimizer = torch.optim.Adam(enc_params, lr=1e-4)
        print(f"Encoder trainable params: {sum(p.numel() for p in enc_params)}")
    else:
        print("⚠️ Encoder has no trainable parameters — training only decoder.")
        enc_optimizer = None
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if enc_optimizer is not None:
            checkpoint["enc_optimizer_state"] = enc_optimizer.state_dict()
        checkpoint["dec_optimizer_state"] = dec_optimizer.state_dict()
        torch.save(checkpoint, ckpt_path)
        print(f"💾 Saved checkpoint: {ckpt_path}")

    # DEMO: inference + optional TTS
    sample_img = None
